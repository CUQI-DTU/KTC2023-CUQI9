...
categoryNbr = 1
print(f"difficult = {categoryNbr}", flush=True)
...
def save(folder, objectNr, solution, segmentation, score, reg1, reg2):
    np.savez(f"results//{folder}//{objectNr}+{reg1}+{reg2}", objectNr = objectNr,
               solution = solution,
...
#reg2_par_list = [0, 8e6]#[0.5, 0.1, 0.05, 0.01] #[0.5, 1, 5, 10, 20]
reg1_par_list = [1, 0.1]
reg2_par_list = np.logspace(1,12,30)#[0, 8e6, 1e7, 6e6, 4e6, 2e6, 1e6]
L = smprior.L
m = Mesh.g.shape[0]
...
for objectno in range(0,len(mat_files)): #compute the reconstruction forc each input file
    ...
    for (ind1, reg1) in enumerate(reg1_par_list):
        for (ind2, reg2) in enumerate(reg2_par_list):
            ...
            print(f"diff = {categoryNbr}, obj_num = {objectno}, reg1_par = {reg1}, reg2_par = {reg2}, score = {score}", flush=True)
            
            save(str(categoryNbr), objectno, deltareco_pixgrid, reconstruction, score, reg1, reg2)
