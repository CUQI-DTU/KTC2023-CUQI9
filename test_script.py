...
for idx, fileName in enumerate(DATA_FILES):
    ...
    mesh, subdomains = create_disk_mesh(radius, electrode_count, polygons=300, fineness=30)
    forward_model = FenicsForwardModel(mesh, subdomains,electrode_count, impedance, background_conductivity)
    
    boundary_gap = 0.1
    reconstruction_mesh, _ = create_disk_mesh(radius - boundary_gap, electrode_count, polygons=20, fineness=7)
